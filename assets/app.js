...
function setRail(html){railNav.innerHTML=html}
...
app.innerHTML=`<div class="content-shell"><header class="reader-head"><div class="eyebrow">Document ${d.roman} · ${si+1} / ${d.sections.length}</div><h1>${esc(d.short)}</h1><div class="reader-meta"><span>${esc(s.title)}</span><a href="${d.sourcePdf}" target="_blank" rel="noopener">PDF ↗</a><a href="${d.sourceMd}" target="_blank" rel="noopener">Markdown ↗</a></div><div class="progress"><i style="width:${((si+1)/d.sections.length)*100}%"></i></div></header>${s.part?`<div class="part-stamp">${esc(s.part)}</div>`:''}<article class="doc-content">${s.html}</article><nav class="doc-nav">${prevHref?`<a href="${prevHref}">← ${esc(prev.title)}</a>`:'<span></span>'}${nextHref?`<a href="${nextHref}">${esc(next.title)} →</a>`:''}</nav></div>`;window.scrollTo(0,0);focusApp();}
...
app.innerHTML=`<div class="content-shell"><header class="primer-intro"><div class="translation-tag">Plain-language companion · ${pi+1} / ${DATA.dummies.length}</div><h1>${esc(p.title)}</h1>${numberedAs?`<p class="numbering-note">(Hypotheses numbered as in ${numberedAs}.)</p>`:''}</header><div class="primer-grid"><nav class="primer-menu">${DATA.dummies.map((x,i)=>`<a class="${i===pi?'active':''}" href="#/primer/${x.id}">${esc(x.title)}</a>`).join('')}</nav><div class="primer-page"><article class="doc-content">${primerBody(p)}</article>${p.id==='equations'?instrumentsHTML():''}<nav class="doc-nav">${pi>0?`<a href="#/primer/${DATA.dummies[pi-1].id}">← ${esc(DATA.dummies[pi-1].title)}</a>`:'<span></span>'}${pi<DATA.dummies.length-1?`<a href="#/primer/${DATA.dummies[pi+1].id}">${esc(DATA.dummies[pi+1].title)} →</a>`:''}</nav></div></div></div>`;if(p.id==='equations')bindInstruments();window.scrollTo(0,0);focusApp();}
...
function render(){closeMenu();closeAtlasLightbox();let r=route();if(!r.length||r[0]==='home')return home();if(r[0]==='document')return docReader(r[1]||0,r[2]||'frontpiece');if(r[0]==='primer')return primer(r[1]||'begin-here');if(r[0]==='atlas')return atlas();home()}
...
function search(q){q=q.trim().toLowerCase();if(q.length<2){results.innerHTML='<div class="search-empty">Type at least two characters.</div>';return}let hits=[];DATA.docs.forEach((d,di)=>d.sections.forEach(s=>{let text=s.title+' '+searchableText(s.html),pos=text.toLowerCase().indexOf(q);if(pos>=0)hits.push({href:`#/document/${di}/${s.id}`,title:`${d.roman}. ${d.short} — ${s.title}`,snippet:text.slice(Math.max(0,pos-90),pos+170)})}));DATA.dummies.forEach(p=>{let text=p.title+' '+searchableText(p.html),pos=text.toLowerCase().indexOf(q);if(pos>=0)hits.push({href:`#/primer/${p.id}`,title:`For Dummies — ${p.title}`,snippet:text.slice(Math.max(0,pos-90),pos+170)})});hits=hits.slice(0,30);results.innerHTML=hits.length?hits.map(h=>`<a class="search-result" href="${h.href}" onclick="closeSearch()"><b>${esc(h.title)}</b><span>${esc(h.snippet)}</span></a>`).join(''):'<div class="search-empty">No match in the dossier.</div>'}
